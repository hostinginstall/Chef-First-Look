...
},SPI:AJS.$.extend({},{BASE_URL:AJS.contextPath()+"/rest/applinks/1.0",getAllLinks:function(E,D){var C=AppLinks.SPI.BASE_URL+"/applicationlink";
AppLinks.get(C,E,D)
...
AppLinks.get(C,E,D)
},showCreateEntityLinkSuggestion:function(){return true
...
return F
},handleUALManifest:function(F,E){var C=A(E);
C.find(".remote-app-image").removeClass(function(J,L){var K=L.split(" ");
var I="";
A.each(K,function(M,N){if(N.indexOf("application-type-image-")!=-1){I=N
}});
return I
});
C.find(".remote-app-image").addClass("application-type-image-"+F.typeId);
C.find(".link-to-app-type").html(AppLinks.I18n.getText("applinks.create.title.link.to",AppLinks.I18n.getApplicationTypeName(F.typeId)));
C.find(".remote-app-name").html(AppLinks.UI.shortenString(F.name,20));
C.find(".create-reciprocal-link").attr("checked",true);
C.find("#reciprocal-link-back-to-server").html(AppLinks.I18n.getText("applinks.create.link.back.to.server",F.name));
var D=["administrator",F.name,'<a target="_blank" href="'+AppLinks.Docs.getDocHref("applinks.docs.adding.application.link")+'">',"</a>"];
var H=F.applinksVersion.split(".");
var B=parseInt(H[0]);
var G=parseInt(H[1]);
if((F.typeId=="jira"||F.typeId=="confluence")&&(B==3&&G<10)){D[0]="system administrator"
}C.find(".reciprocal-link-description").html(AppLinks.I18n.getText("applinks.create.two.way.link",D));
C.find(".no-reciprocal-link-description").hide();
C.find(".no-reciprocal-link-description").html(AppLinks.I18n.getText("applinks.create.two.way.no.link",F.name));
C.find(".reciprocal-link-username").val("");
C.find(".reciprocal-link-password").val("");
C.find(".ual-arrow").removeClass("no-background");
C.find(".two-way-link-details").show();
C.find(".reciprocal-link-description").show();
C.find(".no-reciprocal-link-description").hide()
...
}},checkReciprocalLinkFormThreeStepMode:function(C,D,G,J,F){var H=A(C);
...
return false
}},checkReciprocalLinkFormTwoStepMode:function(F,H,B,I){var C=A(F);
var G=A.trim(C.find(".reciprocal-rpc-url").val());
if(G==""){A("<div class='error applinks-error'>"+AppLinks.I18n.getText("applinks.error.url.field.empty")+"</div>").insertAfter(C.find(".reciprocal-rpc-url"));
if(I){I()
}return 
}var D=C.find(".reciprocal-link-username");
var E=C.find(".reciprocal-link-password");
if(D.val()==""){A('<div class="error applinks-error">'+AppLinks.I18n.getText("applinks.error.username.empty")+"</div>").insertAfter(D);
if(I){I()
}return false
}G=AppLinks.UI.addProtocolToURL(G);
AppLinks.SPI.verifyTwoWayLinkDetails(H,G,D.val(),E.val(),B,AppLinks.UI.displayValidationError("two-way-link-errors",F,I));
return false
}}
})(AJS.$);
} catch (err) {
